...
Gate crossings and ADR acceptances are recorded in two coupled places:

1. **Artifact frontmatter** — each governed artifact carries `decided-by` / `approved-by` / `approved-at`, set only via `/approve`. This is the per-artifact record of who decided and when.
2. **Git history** — the commit that records the approval is the authoritative, append-only audit trail. Commits cannot be silently rewritten without rewriting history, so the log of decisions stays coherent across time.

This pairing replaces any separate log file: the frontmatter answers *what is the current decision state of this artifact*, and git answers *when and by whom was every decision made*.
...
Human approval flows through `/approve <artifact-id>`, which is the only command authorized to set decision fields.
...
- *"Every decision this quarter?"* → `git log --grep="Decided-By"` filtered by date
