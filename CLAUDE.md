...
=======
13. **Questions go through AskUserQuestion** — Every user-facing question (gate or not) uses `AskUserQuestion` with 2–4 concrete clickable options and trade-offs in the `description` field; free-text questions buried in prose are forbidden (see [guidelines/ask-user-question.md](guidelines/ask-user-question.md))
>>>>>>> Stashed changes
