...
- **G3** — every PRD acceptance criterion satisfied with evidence, build passes, docs updated, `insights.md` updated with implementation surprises (not just copied from G2), human-approval line in `governance-log.md`, **no code outside the approved PRD scope** (surprise refactors are an automatic block).
