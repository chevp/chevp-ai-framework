---
description: Override a Gatekeeper "block" verdict on a plan with an explicit reason. Documents the override in the plan's frontmatter.
...
3. **Locate the latest Gatekeeper verdict** for this plan in the conversation or in the plan's existing `gatekeeper-override:` history. If no recent `block` verdict exists, STOP and ask the human to run `/gate-check` first — overrides only apply to actual blocks.
...
5. **Do NOT modify** `decided-by` / `approved-by` / `approved-at`. An override does not approve the plan — it only allows progression. The human still has to run `/approve` separately to record the approval.

6. **Show the diff** to the human before writing. After writing, remind: "Override recorded. Run `/approve <plan-id>` separately to record the approval."
...
- Querying overrides: `grep -rl "gatekeeper-override:" context/plans/` lists every plan carrying an override block for audit.
